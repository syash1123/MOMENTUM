...
for ticker, df in data.items():
    ...
    if len(df) >= 252:
        ...
        if (df['Close'].iloc[-1] >= df['EMA100'].iloc[-1] >= df['EMA200'].iloc[-1] and 
            ...
            return_12m = (df['Close'].iloc[-1] / df['Close'].iloc[-252] - 1) * 100
            return_6m = (df['Close'].iloc[-1] / df['Close'].iloc[-126] - 1) * 100
            # return_1m = (df['Close'].iloc[-1] / df['Close'].iloc[-21] - 1) * 100
...
df_summary['Final_Rank'] = 0.5*df_summary['Rank_12M'] + 0.5*df_summary['Rank_6M'] 
...
#create a table 
col_table =['Ticker','Return_6M', 'Return_3M','Return_1M','Rank_6M','Rank_3M','Rank_1M','Final_Rank','POSITIN']
table = ax.table(cellText =tbl_data,colLabels =col_labels,cellLOc='Center',loc='center')
table.auto_set_font_size(False)
table.auto_set_font_size(12)
table.scale(1,1.5)
